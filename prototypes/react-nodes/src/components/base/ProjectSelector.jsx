 ...
 * 新建(项目名称)/重命名(项目名称)弹窗 `bg-[#2a2a2a] w-64`。
 */
    ...
    <div ref={wrapRef} className="flex items-center gap-1 group/project-selector relative">
          ...
          {projects.map((e) => {
            ...
            return (
              <div
                ...
                className={`px-3 py-2.5 text-sm cursor-pointer flex items-center gap-2 hover:bg-[#333] transition-colors ${isActive ? 'text-white bg-[#222]' : 'text-gray-400'}`}
              >
      ...
      <div className="relative group/project-menu -ml-1 z-10">
        ...
        <div className="absolute left-0 top-full mt-2 w-40 bg-[#1a1a1a] border border-[#333] rounded-xl shadow-2xl opacity-0 invisible group-hover/project-menu:opacity-100 group-hover/project-menu:visible transition-all duration-200 z-float overflow-hidden py-1">
          ...
          <button type="button" onClick={() => window.dispatchEvent(new CustomEvent('import-project'))} className="w-full text-left px-3 py-2.5 text-sm text-gray-300 hover:bg-[#333] hover:text-white flex items-center gap-2 cursor-pointer border-none bg-transparent">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="7 10 12 15 17 10" /><line x1="12" y1="15" x2="12" y2="3" /></svg>
            导入项目
          </button>
          <button type="button" onClick={() => window.dispatchEvent(new CustomEvent('export-project'))} className="w-full text-left px-3 py-2.5 text-sm text-gray-300 hover:bg-[#333] hover:text-white flex items-center gap-2 cursor-pointer border-none bg-transparent">
          ...
          {projects.length > 1 && (
            <>
              ...
              <button type="button" onClick={handleDelete} className="w-full text-left px-3 py-2.5 text-sm text-red-400 hover:bg-[#333] hover:text-red-300 flex items-center gap-2 cursor-pointer border-none bg-transparent">
                <Trash2 size={14} /> 删除项目
              </button>
            </>
          )}
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-modal" onClick={() => setModal(null)}>
          <div className="bg-[#2a2a2a] p-4 rounded-lg border border-[#333] w-64" onClick={(e) => e.stopPropagation()}>
            ...
            <input
              ...
              className="w-full bg-[#151414] border border-[#333] rounded p-2 text-gray-200 text-xs mb-3 focus:outline-none focus:border-blue-500"
            />
