 ...
 * 设置主框架（侧栏 + 舞台）。
 * 风格照抄一毛官方 Vr.jsx 设置页：
 *   - 容器 absolute inset-0 bg-[#0d0c0c]（覆盖画布，纯黑）
 *   - 侧栏 w-48，导航项激活 text-blue-500 font-bold border-[#333] bg-[#252525]
 *   - 内容区 p-6 bg-[#0d0c0c]，内部 max-w-4xl 卡片布局
 */
const SECTIONS = [
...
export default function SettingsFrame() {
  ...
  return (
    <div className="absolute inset-0 flex bg-[#0d0c0c] overflow-hidden z-[100]">
      ...
      <aside className="w-48 bg-[#0d0c0c] border-r-0 flex flex-col p-3 z-10 flex-shrink-0">
        ...
        {SECTIONS.map((s) => {
          ...
          return (
            <button
              ...
              className={`text-left px-3 py-2.5 rounded-lg text-sm transition-colors mb-1.5 flex items-center gap-2 ${active === s.key ? 'bg-[#252525] text-blue-500 border border-[#333] shadow-sm' : 'text-gray-300 hover:bg-[#222] hover:text-gray-100 border border-transparent'}`}
            >
              <Icon size={16} />
        ...
        <div className="mt-auto">
          <button
            ...
            className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors mb-1.5 flex items-center gap-2 ${moreOpen ? 'bg-[#252525] text-blue-500 border border-[#333] shadow-sm' : 'text-gray-300 hover:bg-[#222] hover:text-gray-100 border border-transparent'}`}
          >
      ...
      {/* 内容区 */}
      <main className="flex-1 overflow-y-auto p-6 relative pb-24 custom-scrollbar bg-[#0d0c0c] nowheel nopan nodrag">
