...
/**
 * 设置分区 · 快捷键
 * 风格照抄一毛官方设置页卡片（bg-[#1a1a1a] rounded-xl border-[#222]，头 p-4 border-b）。
...
export default function ShortcutSettings() {
  ...
  return (
    <div className="group bg-[#1a1a1a] rounded-xl overflow-hidden transition-all duration-300 pb-4 shadow-sm border border-[#222]">
      {/* 卡片头 */}
      ...
      <div className="px-4 space-y-2 pt-4">
        {items.map((it) => (
          <div key={it.id} className="flex items-center gap-3 bg-[#222] rounded-lg p-2 group/item border border-transparent hover:border-[#333] transition-colors">
            <div className="flex-1">
              <input
                value={it.action}
                onChange={(e) => updateAction(it.id, e.target.value)}
                placeholder="动作名称"
                className="w-full bg-transparent border-b border-[#444] px-1 py-1.5 text-xs focus:border-blue-500 outline-none placeholder-gray-600 transition-colors text-gray-200"
              />
            </div>
            <div className="w-[180px]">
              <input
                ...
                placeholder="组合键"
                className="w-full bg-[#0d0c0c] border border-[#333] rounded px-2 py-1.5 text-xs text-center text-gray-300 focus:border-blue-500 outline-none transition-colors"
              />
            </div>
            <button
