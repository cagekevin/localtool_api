 ...
 *
 * 官方 Ti 组件（src/bundle/httpClient-BknZwXjG_components/Ti.jsx）+ Di 组件（Di.jsx）：
 * - 背景：backgroundUrl 作为背景，blur-xl scale-110 opacity-25 + bg-black/30 压暗；
 * - 扫光：一条半透明白斜条 npbShimmer 2.2s 从左到右循环扫过；
 * - 右上角状态区：label（如"生图中..."）+ 旋转 SVG 圆环加载器（npbSpin 0.9s）
 *   + 背后径向呼吸光晕（npbGlow 1.8s）；
 * - 左下角：灵感 Tips 轮播（Di），按 category 过滤，10s 一条，淡入上滑。
 *
 * 接真系统路径：label/backgroundUrl/category 语义与官方一致，直接替换即可。
 *
 * @param {object} props
...
const GEN_TIPS = [
  // text
  { text: '提问前加上"你是一位资深文案"，AI的输出结构会更专业', category: 'text' },
  { text: '告诉AI"请使用积极的语气"，比说"不要用消极语气"效果更好', category: 'text' },
  { text: '在提示词中附带满意的案例，AI能迅速模仿你的行文格式', category: 'text' },
  { text: '加上"请一步步进行推理"，能大幅提高处理复杂逻辑题的准确率', category: 'text' },
  { text: '交待清楚目标受众和具体应用场景，生成的文案会更有针对性', category: 'text' },
  { text: '不要让AI一次写完长文，先生成大纲，确认后再逐段扩写', category: 'text' },
  { text: '设定具体的字数和情绪，如"写一段100字幽默带讽刺的短评"', category: 'text' },
  { text: '输入长文并要求"提取时间、地点、人物，并以JSON格式输出"', category: 'text' },
  { text: '把优秀的文案喂给AI，让它分析并反推当初生成这段文案的提示词', category: 'text' },
  ...
  // image
  { text: '把最重要的元素（如人物、主要物体）放在提示词的最开头位置', category: 'image' },
  { text: '加入"电影级光效"、"丁达尔效应"或"边缘背光"提升画面高级感', category: 'image' },
  { text: '使用"广角镜头"、"微距特写"或"俯视仰拍"精准控制画面构图', category: 'image' },
  { text: '提示词中加入"莫兰迪色系"、"高对比度"统一画面的色彩倾向', category: 'image' },
  { text: '添加"杰作、最高画质、8k分辨率、细节极其丰富"等通用魔法词', category: 'image' },
  { text: '利用参考图控制构图走势，配合文本提示词进行二次风格迁移', category: 'image' },
  { text: '描述细节忌抽象：说"穿红裙在雨中撑伞的女孩"，不要说"忧郁女孩"', category: 'image' },
  { text: '大尺寸慢不稳而且贵，可以先生成小尺寸，满意后高清放大处理', category: 'image' },
  ...
  { text: '拆解动作过程，如"他先低头看手表，然后慢慢抬头望向天空"', category: 'video' },
  { text: '对于长视频，提供多视角的参考图，能有效减少过程中的人物崩坏', category: 'video' },
  { text: '描述动作和场景即可，太复杂的心理描写AI视频模型目前无法表现', category: 'video' },
  { text: '添加光影动态变化，如"阳光透过树叶缝隙，光斑在人物脸上移动"', category: 'video' },
  { text: '先用极高画质的模型生成图像，再输入到视频模型让图片动起来', category: 'video' },
  { text: '根据低端模型模型能力选择5秒生成，过长的时间容易导致后半段画面崩塌', category: 'video' },
  { text: '部分模型支持音频节点，让可以让生成的数字人根据台词音频精准对口型', category: 'video' },
  { text: '描述"大雪纷飞"、"烟雾弥漫"，这类动态粒子效果AI处理极为出色', category: 'video' },
  { text: '加入"频繁眨眼"、"嘴角微微上扬"，让生成的视频人物更有生命力', category: 'video' },
  { text: '单个短镜头内尽量保持单一视角，复杂的机位切换容易导致空间错乱', category: 'video' },
  { text: '写长视频脚本时，务必把提示词按照场景分开，建立独立节点生成', category: 'video' },
  { text: '上传真人动作视频作为骨骼参考，让AI角色完美复刻复杂的舞蹈动作', category: 'video' },
  { text: '打斗跳舞高运动感模型目前只推荐SD2', category: 'video' },
  ...
  { text: '在复杂的节点群旁边添加文本便签，几个月后你依然能一眼看懂逻辑', category: 'general' },
  { text: '工作流会被实时保存在本地，即使意外关闭浏览器，进度也绝不会丢失', category: 'general' },
