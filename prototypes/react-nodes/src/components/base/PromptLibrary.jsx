    ...
    <div className="fixed inset-0 z-modal bg-black/75 backdrop-blur-sm flex items-center justify-center p-6 nowheel nopan nodrag" onClick={onClose}>
      <div className="w-[88vw] h-[84vh] max-w-[1400px] bg-[#141414] border border-[#2a2a2a] rounded-[18px] shadow-2xl flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="h-[60px] border-b border-[#222] flex items-center px-5 gap-4 flex-shrink-0">
          ...
          <div className="relative flex-1 max-w-[320px]">
            ...
            <input
              ...
              className="w-full h-[34px] bg-[#1a1a1a] border border-[#333] rounded-[10px] pl-9 pr-3 text-[#e5e5e5] text-[13px] outline-none focus:border-[#555] box-border"
            />
          </div>
          <button className="ml-auto w-8 h-8 flex items-center justify-center bg-transparent hover:bg-[#2a2a2a] rounded-lg text-[#888] hover:text-white cursor-pointer" onClick={onClose} title="关闭">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* 左侧导航 */}
          <div className="w-[170px] border-r border-[#222] p-4 pr-3 flex flex-col gap-1.5 flex-shrink-0">
            <button
              className={`flex items-center gap-2.5 px-3 py-2.5 rounded-[10px] text-[13px] transition-all cursor-pointer text-left border-none ${activeTab === 'mine' ? 'bg-[#1f1f1f] text-white font-medium' : 'text-[#999] hover:bg-[#1f1f1f] hover:text-[#ddd] bg-transparent'}`}
              onClick={() => setActiveTab('mine')}
            >
              <List size={16} /> 我的提示词
            </button>
            <button
              className={`flex items-center gap-2.5 px-3 py-2.5 rounded-[10px] text-[13px] transition-all cursor-pointer text-left border-none ${activeTab === 'recent' ? 'bg-[#1f1f1f] text-white font-medium' : 'text-[#999] hover:bg-[#1f1f1f] hover:text-[#ddd] bg-transparent'}`}
            ...
            <div className="h-[58px] px-5 flex items-center justify-between gap-3 flex-shrink-0">
              <div className="flex gap-2 items-center">
                {CATEGORY_OPTIONS.map((opt) => (
                  <button
                    key={opt.value || 'all'}
                    className={`px-3.5 py-1.5 rounded-full text-xs transition-all cursor-pointer border ${selectedCategory === opt.value ? 'bg-white text-[#141414] font-medium border-white' : 'bg-[#1f1f1f] text-[#999] border-transparent hover:bg-[#2a2a2a] hover:text-[#ddd]'}`}
              ...
              {displayCards.length === 0 ? (
                ...
                <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-3.5">
                  {displayCards.map((card) => (
                    <div
                      key={card.id}
                      className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-[14px] p-4 flex flex-col gap-2.5 cursor-pointer transition-all hover:border-[#444] hover:bg-[#1f1f1f]"
                    ...
                    >
                      <div className="flex items-start justify-between gap-2.5">
                        ...
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity" style={{ display: 'flex', opacity: 0 }}>
                          <button
                            className="w-[26px] h-[26px] flex items-center justify-center rounded-md bg-transparent hover:bg-[#2a2a2a] text-[#888] hover:text-white cursor-pointer border-none"
                            title="编辑"
                          ...
                          <button
                            className="w-[26px] h-[26px] flex items-center justify-center rounded-md bg-transparent hover:bg-red-500/10 text-[#888] hover:text-red-400 cursor-pointer border-none"
                            title="删除"
                      ...
                      <p className="text-xs text-[#888] leading-[1.6] m-0" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                        {card.content || '(空)'}
                      </p>
                      <div className="flex items-center justify-between mt-1">
                        <span className={`px-2 py-0.5 rounded-md text-[11px] font-medium ${TYPE_TAG_CLASS[card.category] || 'bg-white/10 text-[#aaa]'}`}>
                          {card.category ? TYPE_LABEL[card.category] : '通用'}
                        </span>
                        <button
                          className="px-3 py-1 rounded-lg text-xs text-blue-400 border border-[#333] bg-transparent hover:bg-blue-500/10 hover:border-blue-500/30 cursor-pointer transition-all"
          ...
          <div className="w-[520px] bg-[#1a1a1a] border border-[#333] rounded-2xl p-6 flex flex-col gap-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2.5">
              <div className="flex flex-col gap-1.5 flex-[2]">
                <label className="text-xs text-[#888]">标题</label>
                <input
                  placeholder="标题"
              ...
              <div className="flex flex-col gap-1.5 flex-1">
                <label className="text-xs text-[#888]">类型</label>
                <select
                  value={formData.type}
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs text-[#888]">提示词内容</label>
              <textarea
                placeholder="提示词内容"
                value={formData.prompt}
                onChange={(e) => setFormData((d) => ({ ...d, prompt: e.target.value }))}
                className="bg-[#141414] border border-[#333] rounded-[10px] px-3 py-2.5 text-[#e5e5e5] text-[13px] outline-none focus:border-[#555] resize-none h-[160px] leading-[1.6] box-border"
              />
            </div>
            <div className="flex items-center justify-between mt-1">
              {!showNewForm ? (
              ...
              <div className="flex gap-2.5">
                <button className="px-4 py-2 rounded-[10px] text-xs bg-[#2a2a2a] text-[#ccc] hover:bg-[#333] cursor-pointer border-none" onClick={() => { setEditingIndex(-1); setShowNewForm(false) }}>取消</button>
