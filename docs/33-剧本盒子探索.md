...
> **变更记录（2026-08-07，重打补丁依据）**：
> - **改动文件/chunk**：`src/bundle/httpClient-BknZwXjG_components/shared.js`（构建回灌 `dist/assets/httpClient-BknZwXjG.js`）。
> - **原始符号**：`Fa`（资产名匹配函数，原 L2882）。
> - **改动点**：`Fa` 内 `indexOf(`@${t}`)` 单次命中 → 改为 `while` 循环逐位置扫描，任一命中后一位合法即 true。
> - **目的**：修复「文本同时含 `@小马妈妈` 与 `@小马` 时，`@小马` 漏匹配」——旧版 `indexOf` 只查首个命中（撞上 `@小马妈妈` 后一位「妈」非法即放弃），漏掉后面独立的 `@小马`，导致该资产参考图带不进下游。
> - **影响行为**：仅补上报（漏报→命中），防误报方向不变（`@小马妈妈` 配资产「小马」、`@小红帽子` 配资产「小红帽」仍 false）；`Fa` 输入输出契约（返回 boolean）不变，调用方 `Ra`/`bo.jsx`/`As.jsx`/`ii` 无感。
> - **验证**：`npm run build` 通过；7 个边界用例（防误报/修复漏报/末尾边界/无引用）全 PASS。
> - **副作用边界**：无死循环（`indexOf` 每次前进 ≥1）；正常单命中场景只扫一次，性能无感。
