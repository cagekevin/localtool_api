#!/usr/bin/env node
/**
 * summarize.cjs — AST 驱动的源码速读器
 *
 * 用法:
 *   node scripts/summarize.cjs <文件路径> --summary     # import/export 速览（gougou 常用）
 *   node scripts/summarize.cjs <文件路径> --deps        # 依赖图分析
 *   node scripts/summarize.cjs <文件路径> --unlock      # 瓶颈分析
 *   node scripts/summarize.cjs <文件路径> <函数名>      # 单函数摘要
 *   node scripts/summarize.cjs <文件路径> --all         # 全部函数
 *   node scripts/summarize.cjs <文件路径> --skeleton    # JSX 骨架
 *
 * gougou 场景示例:
 *   node scripts/summarize.cjs src/legacy/endpointConfig-Bt85xi8d.js --summary
 *   node scripts/summarize.cjs src/legacy/httpClient-Bqba_SHR.js --summary
 *   node scripts/summarize.cjs src/legacy/ShareAppPage-BVCmVrHF.js --deps
 *
 * 源自 maomao 项目，已适配 gougou（接受任意文件路径 + --summary 模式）。
 */
...
const SRC_PATH = positionalArgs.length > 0 && !positionalArgs[0].startsWith("-")
  ? positionalArgs[0]
  : "src/App.js";
...
// ── summary 模式：仅输出 import/export 速览 ──
if (args.includes("--summary")) {
  const shortName = SRC_PATH.replace(/^.*[\\/]/, '');
  console.log(`\n📄 ${shortName}  |  ${src.split('\n').length} 行  |  ${(Buffer.byteLength(src) / 1024).toFixed(1)} KB\n`);
  ...
  for (const node of ast.body) {
    if (node.type === "ImportDeclaration") {
      ...
      const names = node.specifiers.map(s => {
        ...
        return s.imported.name === s.local.name ? s.local.name : `${s.imported.name}→${s.local.name}`;
      });
      if (!importsBySource.has(source)) importsBySource.set(source, []);
      importsBySource.get(source).push(...names);
    }
  }

  if (importsBySource.size > 0) {
    console.log('── import（从哪些 chunk 引入）');
    for (const [src, names] of importsBySource) {
      const basename = src.replace(/^.*[\\/]/, '');
      console.log(`  ← ${basename}  (${names.length} 项)`);
    }
    console.log();
  }
  ...
  for (const node of ast.body) {
    if (node.type === "ExportNamedDeclaration") {
      for (const spec of node.specifiers || []) {
        ...
        namedExports.push(as === from ? as : `${from}→${as}`);
      }
    }
    if (node.type === "ExportAllDeclaration") {
      starExports.push(node.source.value.replace(/^.*[\\/]/, ''));
    }
    if (node.type === "ExportDefaultDeclaration") {
      defaultExports.push(1);
    }
  }

  if (starExports.length + namedExports.length + defaultExports.length > 0) {
    console.log('── export（对外暴露了什么）');
    for (const s of starExports) console.log(`  ★  export * from '${s}'`);
    if (defaultExports.length > 0) console.log(`  →  export default (${defaultExports.length} 个)`);
    if (namedExports.length > 0) {
      console.log(`  →  ${namedExports.length} 个具名导出: ${namedExports.slice(0, 20).join(', ')}${namedExports.length > 20 ? ' …' : ''}`);
    }
    console.log();
  }
...
// 目标过滤处理
const filterArgs = positionalArgs.slice(1); // 跳过文件路径
